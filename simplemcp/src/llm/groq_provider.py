...
class GroqProvider(LLMProvider):
    def __init__(self, session: ClientSession, model: str = "llama-3.1-70b-versatile", api_key: str = None):
        super().__init__(session)
        self.model = model
        self.client = Groq(api_key=api_key)  # Uses GROQ_API_KEY env var if api_key is None

    def format_tools(self, tools) -> List[Dict[str, Any]]:
        """Format MCP tools for Groq's expected format"""
        return [{
            "type": "function",
            "function": {
                "name": tool.name,
    ...
    async def ask(self, query: str) -> str:
        ...
        try:
            ...
            response = await self._call_groq(messages, available_tools)
            return await self._process_groq_response(response, messages, available_tools)

        except Exception as e:
            logger.error(f"Error in Groq processing: {e}")
            return f"Error processing query with Groq: {str(e)}"

    async def _call_groq(self, messages: List[Dict], tools: List[Dict] = None) -> Dict:
        ...
        try:
            ...
            logger.warning(f"Groq tool calling failed, trying without tools: {e}")
    ...
    async def _process_groq_response(self, response, messages: List[Dict], available_tools: List[Dict]) -> str:
        """Process Groq response and handle tool calls"""
        final_text = []
        ...
        if message.tool_calls:
            for tool_call in message.tool_calls:
                try:
                    ...
                    follow_up_response = await self._call_groq(messages, available_tools)
